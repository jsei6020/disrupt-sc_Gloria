...
# Python code to transform GLORIA MR-SUT to IOT
# This code implements multiple transformation methods (A, B, C, D)
...
class GLORIATransformer:
    ...
    def _validate_dimensions(self):
        ...
        assert self.V.shape == (self.n_total, self.n_total), \
            f"Supply table shape mismatch: {self.V.shape} != ({self.n_total}, {self.n_total})"
        assert self.U.shape == (self.n_total, self.n_total), \
            f"Use table shape mismatch: {self.U.shape} != ({self.n_total}, {self.n_total})"
        assert self.Y.shape[0] == self.n_total, \
            f"Final demand rows mismatch: {self.Y.shape[0]} != {self.n_total}"
        assert self.W.shape[1] == self.n_total, \
            f"Value added columns mismatch: {self.W.shape[1]} != {self.n_total}"
...
with open('gloria_sut_to_iot.py', 'w') as f:
    f.write(code_content)

print("Python code generated and saved to: gloria_sut_to_iot.py")
print("\nCode includes:")
print("- GLORIATransformer class with all 4 transformation methods (A, B, C, D)")
print("- Handles 164 regions x 97 sectors = 15,908 dimensions")
print("- Safe handling of divisions by zero")
print("- Technical coefficient and Leontief inverse computation")
print("- Complete working example with test data")
print("\nFile size: {} characters".format(len(code_content)))
